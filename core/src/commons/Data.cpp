...
void Data::set_outcome_index(std::vector<size_t> index) { //
  ...
  this->outcome_index.resize(index.size());
  for (size_t i = 0; i < index.size(); ++i) {
    this->outcome_index[i] = index[i];
  }
  
  for (size_t i = 0; i < this->outcome_index.size(); ++i) {
    disallowed_split_variables.insert(this->outcome_index[i]);
  }
...
std::vector<double> Data::get_outcome(size_t row) const { //
 // std::cout << outcome_index.size() << std::endl;
  std::vector<double> out;
  for (size_t d = 0; d < outcome_index.size(); ++d) {
    out.push_back(get(row, outcome_index[d]));
  }
  return out;
}
